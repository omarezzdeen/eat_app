...
class Foods with ChangeNotifier {
  ...
  getAllFoods(){
    final Stream<QuerySnapshot> foodStream = FirebaseFirestore.instance.collection('foods').orderBy('rating').snapshots();
    return foodStream;
  }

  addNewItemFood(Food food){
    try{
      firestore.collection('foods').add(food.toJson());
    }catch(e){
